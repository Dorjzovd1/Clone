...
export const api = {
  ...
  listAudit: (params?: { case_id?: number; limit?: number }) => {
    const qs = new URLSearchParams();
    if (params?.case_id) qs.append("case_id", String(params.case_id));
    if (params?.limit) qs.append("limit", String(params.limit));
    const q = qs.toString();
    return http<AuditLog[]>(`/audit${q ? `?${q}` : ""}`);
  },
  ...
  findingMetadata: (id: number) => http<FindingMetadata>(`/findings/${id}/metadata`),
  downloadUrl: (id: number) => `${BASE}/findings/${id}/download`,

  // Metadata / Forensic
  scanNtfsArtifacts: (scanId: number) => http<Finding[]>(`/scans/${scanId}/ntfs-artifacts`),
  scanUsnEvents: (scanId: number) => http<Finding[]>(`/scans/${scanId}/usn-events`),
  forensicTimeline: (scanId: number) => http<TimelineEvent[]>(`/scans/${scanId}/forensic-timeline`),
  metadataCsvUrl: (scanId: number) => `${BASE}/scans/${scanId}/metadata/csv`,
  forensicTools: () => http<{ tools: ForensicToolInfo[]; ntfs_artifacts: string[] }>("/forensic/tools"),
