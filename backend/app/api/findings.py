...
def list_findings(
    ...
    category: str | None = Query(None, description="existing | deleted | recovered"),
    ...
    if category == "existing":
        query = query.filter(Finding.finding_type == FindingType.EXISTING_FILE)
        ...
        query = query.filter(Finding.recovered.is_(True))
    if q:
        like = f"%{q}%"
