...
# --------------------------------------------------------------------------- #
# All files (fls — идэвхтэй)
# --------------------------------------------------------------------------- #
_FLS_ALL_PRETTY_RE = re.compile(
    r"^\s*\S+/\S+\s+(?:\*\s+)?([\d-]+):\s*(.+?)\s*$"
)


def list_all_files(source_path: str, byte_offset: int = 0) -> list[FileEntry]:
    """Бүх идэвхтэй файлуудыг TSK `fls`-ээр жагсаана."""
    if not tsk_available():
        if settings.allow_mock:
            return _mock_all_files()
        return []

    by_inode: dict[str, FileEntry] = {}
    for entry in _list_all_body(source_path, byte_offset):
        by_inode[entry.inode] = entry
    for entry in _list_all_pretty(source_path, byte_offset):
        if entry.inode not in by_inode or len(entry.name) > len(by_inode[entry.inode].name):
            by_inode[entry.inode] = entry
    entries = list(by_inode.values())
    logger.info("TSK fls (all): %d идэвхтэй файл (offset=%d)", len(entries), byte_offset)
    return entries


def _list_all_body(source_path: str, byte_offset: int) -> list[FileEntry]:
    args = ["fls", "-r", "-m", "/"]
    if byte_offset:
    ...
    for line in result.stdout.splitlines():
        ...
        if entry and entry.file_type == "r" and not entry.realloc and "(deleted" not in line:
            entries.append(_body_to_file(entry))
    return entries


def _list_all_pretty(source_path: str, byte_offset: int) -> list[FileEntry]:
    args = ["fls", "-r", "-p"]
    if byte_offset:
        args += ["-o", str(byte_offset // 512)]
    args.append(source_path)
    ...
    for line in result.stdout.splitlines():
        if "*" in (line.split(":")[0] if ":" in line else ""):
            continue
        entry = _parse_all_pretty_line(line)
        if entry:
            entries.append(entry)
    return entries


def _body_to_file(entry: DeletedEntry) -> FileEntry:
    return FileEntry(
        inode=entry.inode,
        name=entry.name,
...
def _parse_all_pretty_line(line: str) -> FileEntry | None:
    m = _FLS_ALL_PRETTY_RE.match(line)
    if not m or "*" in line.split(":")[0]:
        return None
    inode, raw_name = m.groups()
    name = raw_name.strip()
    if not name or name in (".", "..") or line.strip().startswith("d/"):
        return None
    path = name if name.startswith("/") else f"/{name}"
    return FileEntry(inode=inode, name=path, file_type="r", size=0, meta={"source": "fls-pretty"})
...
# --------------------------------------------------------------------------- #
def _mock_all_files() -> list[FileEntry]:
    now = datetime.now(timezone.utc)
    return [
        FileEntry(inode="10-128-1", name="/Documents/report.docx", file_type="r", size=102400, mtime=now, crtime=now, meta={"mock": True}),
        FileEntry(inode="11-128-1", name="/Documents/budget.xlsx", file_type="r", size=204800, mtime=now, crtime=now, meta={"mock": True}),
        FileEntry(inode="12-128-1", name="/Photos/vacation.jpg", file_type="r", size=2048000, mtime=now, crtime=now, meta={"mock": True}),
        FileEntry(inode="0-128-1", name="/$MFT", file_type="r", size=262144, mtime=now, meta={"mock": True, "ntfs_system": True}),
        FileEntry(inode="11-128-2", name="/$Extend/$UsnJrnl:$J", file_type="r", size=33554432, mtime=now, meta={"mock": True, "ntfs_system": True}),
    ]
